...
class PIDJoints : public ModelPlugin             
  {
    ...
    sub_3 = n.subscribe("left",1000 , &gazebo::PIDJoints::callback_3, this);
    }
...
void callback_3(const std_msgs::Float64Ptr &msg)
{
  delta_theta = msg -> data;
  drift = delta_theta + drift;
...
common::Time current_time = this->model->GetWorld()->GetSimTime();
    ...
    drift = 0;
      ...
      right_wheel_Torque = right_motor_Torque+ drift;
      left_wheel_Torque = left_motor_Torque + (4*(R_phi-L_phi)+3*(D_R_phi-D_L_phi))-drift;
